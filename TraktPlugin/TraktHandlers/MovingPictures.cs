  ...
  class MovingPictures : ITraktHandler
  {
    ...
    private void ShowRateDialog( DBMovieInfo movie )
    {
      if ( MovingPicturesCore.Settings.AutoPromptForRating )
        return;    // movpics dialog is enabled
      if ( !TraktSettings.ShowRateDialogOnWatched )
        return;             // not enabled
      if ( movie.ActiveUserSettings.UserRating > 0 )
        return;            // already rated
      if ( AdvancedRatings && HasAdvancedRating( movie ) )
        return;        // already rated (advanced)
      ...
      var rateThread = new Thread( ( objMovie ) =>
      {
        ...
        else
        {
          ...
          if ( AdvancedRatings )
            SetAdvancedRating( movieToRate, 0 );
    ...
    {
      if ( node == null || !BasicHandler.IsValidImdb( movieId ) )
        return;
    ...
    static void RemoveMovieCriteriaFromNode( DBNode<DBMovieInfo> node, string movieId )
    {
      ...
      if ( !node.HasFilter || string.IsNullOrEmpty( movieId ) )
        return;
