...
namespace TraktPlugin.TraktHandlers
{
  ...
  class TVSeries : ITraktHandler
  {
    ...
    {
      if ( ( obj as DBEpisode ) != null )
        return SelectedType.Episode;
      if ( ( obj as DBSeries ) != null )
        return SelectedType.Series;
      if ( ( obj as DBSeason ) != null )
        return SelectedType.Season;
    ...
    private void RateShow( DBSeries show )
    {
      ...
      {
        if ( show[ DBOnlineSeries.cTraktIgnore ] )
          return;
    ...
    {
      if ( DBOption.GetOptions( DBOption.cAskToRate ) )
        return;               // tvseries dialog is enabled
      if ( !TraktSettings.ShowRateDialogOnWatched )
        return;                 // not enabled
      if ( episode[ DBOnlineEpisode.cMyRating ] > 0 )
        return;                 // already rated
      if ( isPlaylist && !TraktSettings.ShowRateDlgForPlaylists )
        return;   // disabled for playlists
