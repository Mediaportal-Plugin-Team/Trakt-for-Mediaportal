...
namespace TraktPlugin.GUI
{
  public class GUISettingsPlugins : GUIWindow
  {
    ...
    private bool InitProperties()
    {
      ...
      {
        if ( TVSeries >= 0 )
          btnTVSeries.Selected = true;
        if ( MovingPictures >= 0 )
          btnMovingPictures.Selected = true;
        if ( MyVideos >= 0 )
          btnMyVideos.Selected = true;
        if ( MyFilms >= 0 )
          btnMyFilms.Selected = true;
        if ( OnlineVideos >= 0 )
          btnOnlineVideos.Selected = true;
        if ( MyRecordedTV >= 0 )
          btnMyRecordedTV.Selected = true;
        if ( ArgusRecordings >= 0 )
          btnArgusRecordings.Selected = true;
        if ( MyLiveTV >= 0 )
          btnMyLiveTV.Selected = true;
        if ( ArgusLiveTV >= 0 )
          btnArgusLiveTV.Selected = true;
      }
      catch
      {
        // Skin out of date!
