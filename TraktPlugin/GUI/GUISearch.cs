...
namespace TraktPlugin.GUI
{
  public class GUISearch : GUIWindow
  {
    ...
    IEnumerable<TraktSearchResult> SearchResults
    {
      get
      {
        ...
        if ( !searchCache.Keys.Contains( key ) || LastRequest < DateTime.UtcNow.Subtract( new TimeSpan( 0, TraktSettings.WebRequestCacheMinutes, 0 ) ) )
        {
          ...
          if ( searchCache.Keys.Contains( key ) )
    ...
    protected override void OnPageDestroy( int new_windowId )
    {
      ...
      if ( movieSearchButton.IsSelected() )
        searchTypes |= SearchType.movie;
      if ( showSearchButton.IsSelected() )
        searchTypes |= SearchType.show;
      if ( episodeSearchButton.IsSelected() )
        searchTypes |= SearchType.episode;
      if ( peopleSearchButton.IsSelected() )
        searchTypes |= SearchType.person;
      if ( userSearchButton.IsSelected() )
        searchTypes |= SearchType.user;
      if ( listSearchButton.IsSelected() )
        searchTypes |= SearchType.list;
    ...
    private void SetSearchTypes()
    {
      ...
      if ( movieSearchButton.IsSelected() )
        SearchTypes.Add( SearchType.movie );
      if ( showSearchButton.IsSelected() )
        SearchTypes.Add( SearchType.show );
      if ( episodeSearchButton.IsSelected() )
        SearchTypes.Add( SearchType.episode );
      if ( peopleSearchButton.IsSelected() )
        SearchTypes.Add( SearchType.person );
      if ( userSearchButton.IsSelected() )
        SearchTypes.Add( SearchType.user );
      if ( listSearchButton.IsSelected() )
        SearchTypes.Add( SearchType.list );
    }
    ...
    private void SendSearchResultsToWindow( IEnumerable<TraktSearchResult> searchResults )
    {
      ...
      {
        case SearchType.movie:
          if ( GetSearchResultCount( searchResults, SearchType.movie ) == 0 )
            break;
        ...
        case SearchType.show:
          if ( GetSearchResultCount( searchResults, SearchType.show ) == 0 )
            break;
        ...
        case SearchType.episode:
          if ( GetSearchResultCount( searchResults, SearchType.episode ) == 0 )
            break;
        ...
        case SearchType.person:
          if ( GetSearchResultCount( searchResults, SearchType.person ) == 0 )
            break;
        ...
        case SearchType.user:
          if ( GetSearchResultCount( searchResults, SearchType.user ) == 0 )
            break;
        ...
        case SearchType.list:
          if ( GetSearchResultCount( searchResults, SearchType.list ) == 0 )
            break;
          //TODO
          break;
      }
    ...
    private void InitProperties()
    {
      ...
      if ( ( searchTypes & SearchType.movie ) == SearchType.movie )
        movieSearchButton.Selected = true;
      if ( ( searchTypes & SearchType.show ) == SearchType.show )
        showSearchButton.Selected = true;
      if ( ( searchTypes & SearchType.episode ) == SearchType.episode )
        episodeSearchButton.Selected = true;
      if ( ( searchTypes & SearchType.person ) == SearchType.person )
        peopleSearchButton.Selected = true;
      if ( ( searchTypes & SearchType.user ) == SearchType.user )
        userSearchButton.Selected = true;
