  ...
  internal class TraktDashboard
  {
    ...
    private void PublishMovieProperties( IEnumerable<TraktMovieTrending> trendingItems )
    {
      ...
      for ( int i = 0; i < maxItems; i++ )
      {
        ...
        if ( trendingItem == null || trendingItem.Movie == null )
          continue;
    ...
    private void PublishShowProperties( IEnumerable<TraktShowTrending> trendingItems )
    {
      ...
      {
        var trendingItem = trendingList[ i ];
        if ( trendingItem == null || trendingItem.Show == null )
          continue;
    ...
    {
      if ( activity.Action != ActivityAction.shout.ToString() )
        return string.Empty;
      if ( activity.Shout.IsSpoiler )
    ...
    private void GUIWindowManager_Receivers( GUIMessage message )
    {
      ...
      switch ( message.Message )
      {
        case GUIMessage.MessageType.GUI_MSG_CLICKED:
          ...
          {
            if ( message.Param1 != 7 )
              return; // mouse click, enter key, remote ok, only
        ...
        case Action.ActionType.ACTION_MOVE_DOWN:
          ...
          if ( TraktSettings.MPVersion < new Version( 1, 5, 0, 0 ) )
          {
            if ( !mTrendingContextMenuIsActive && activeWindow.GetFocusControlId() == (int)TraktDashboardControls.TrendingShowsFacade )
            {
              ...
              if ( control.CurrentLayout != GUIFacadeControl.Layout.Filmstrip )
                return;
