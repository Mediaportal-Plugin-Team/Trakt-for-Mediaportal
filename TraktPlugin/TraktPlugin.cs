...
namespace TraktPlugin
{
  ...
  public class TraktPlugin : GUIWindow
  {
    ...
    internal static void SyncPlayback()
    {
      ...
      if ( TraktHandlers.Count == 0 || !TraktSettings.SyncPlayback )
        return;
                ...
                case ( (int)ExternalPluginControls.CustomList ):
                  ...
                  if ( isDetails && ( siteUtil == "imdb" || siteUtil == "itmovietrailers" ) )
                  {
                    ...
                    if ( ( !string.IsNullOrEmpty( title ) && !string.IsNullOrEmpty( year ) ) || imdbid.StartsWith( "tt" ) )
                    {
                      ...
                      if ( message.SenderControlId == (int)ExternalPluginControls.CustomList )
                        validCustomListItem = true;
                    }
            ...
            case (int)ExternalPluginWindows.VideoInfo:
              ...
              switch ( message.SenderControlId )
              {
                ...
                case ( (int)ExternalPluginControls.TraktMenu ):
                  ...
                  if ( people != string.Empty && people != "unknown" )
                    searchPeople.Directors.AddRange( people.Split( ',' ).Select( s => s.Trim() ) );
                  ...
                  if ( !string.IsNullOrEmpty( imdbid ) || ( !string.IsNullOrEmpty( title ) && !string.IsNullOrEmpty( year ) ) )
                  {
                    ...
                    if ( message.SenderControlId == (int)ExternalPluginControls.RelatedItems )
                      validRelatedItem = true;
                    if ( message.SenderControlId == (int)ExternalPluginControls.TraktMenu )
                      validTraktMenuItem = true;
                  }
              ...
              switch ( message.SenderControlId )
              {
                ...
                case ( (int)ExternalPluginControls.TraktMenu ):
                  ...
                  if ( !string.IsNullOrEmpty( imdbid ) || ( !string.IsNullOrEmpty( title ) && !string.IsNullOrEmpty( year ) ) )
                  {
                    ...
                    if ( message.SenderControlId == (int)ExternalPluginControls.TraktMenu )
                      validTraktMenuItem = true;
                    if ( message.SenderControlId == (int)ExternalPluginControls.SearchBy )
                      validSearchItem = true;
                  }
